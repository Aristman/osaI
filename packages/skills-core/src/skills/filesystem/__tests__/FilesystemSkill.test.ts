...
describe('FilesystemSkill', () => {
  ...
  describe('get_file_info', () => {
    it('TC-004-10: should return file metadata', async () => {
      ...
      const data = result as { size: number; type: string };
  ...
  describe('error handling', () => {
    ...
    it('should throw when write_file path is missing', async () => {
      ...
      await expect(handler({ content: 'test' })).rejects.toThrow(
    ...
    it('should throw when move_file source is missing', async () => {
      ...
      await expect(handler({ destination: '/tmp/dest' })).rejects.toThrow(
        'Parameters "source" and "destination" are required',
      );
    });
  });
});
