...
// --- Test Factories ---
function createTool(name, handler) {
    return {
        name,
...
function createSkill(name, tools, overrides) {
    return {
        name,
        version: '1.0.0',
...
describe('SkillRegistry', () => {
    ...
    describe('TC-001-5: execute() with non-existent tool throws error', () => {
        it('should throw an error when executing a non-existent tool', async () => {
            await expect(registry.execute('nonexistent_tool', {})).rejects.toThrow(/nonexistent_tool/);
        });
    });
