...
function createMockServer(): { server: WsServer; sent: ServerMessage[] } {
  const sent: ServerMessage[] = [];
  const server = {
    send: vi.fn((_: string, msg: object) => {
      sent.push(msg as ServerMessage);
    }),
    broadcast: vi.fn(),
...
describe("MessageHandler", () => {
  ...
  describe("handleMessage -- parse and validate", () => {
    it("should parse a valid JSON message and extract fields", async () => {
      const testHandler = vi.fn(async (_clientId: string, msg: ClientMessage): Promise<ServerMessage | void> => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "TEST", id: "msg-1", chatId: "chat-abc", payload: { key: "value" } }),
      );
      ...
      expect(mock.server.send).toHaveBeenCalledWith(
        CLIENT_ID,
        { type: "TEST_REPLY", id: "msg-1", payload: { key: "value" } },
      );
    });
  ...
  describe("PING / PONG", () => {
    it("should respond with PONG for PING message", async () => {
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: MessageType.PING, id: "ping-1" }),
      );
    ...
    it("should respond with PONG without id if id is missing", async () => {
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: MessageType.PING }),
      );
  ...
  describe("unknown type handling", () => {
    it("should return ERROR for unknown message type", async () => {
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "UNKNOWN_TYPE", id: "msg-99" }),
      );
  ...
  describe("handler dispatch", () => {
    it("should dispatch to registered custom handler", async () => {
      const customHandler = vi.fn(async (_clientId: string, msg: ClientMessage): Promise<ServerMessage> => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "MY_ACTION", id: "action-1" }),
      );
    ...
    it("should not send a response when handler returns void", async () => {
      const voidHandler = vi.fn(async (): Promise<void> => {
        // No response
      });
    ...
    it("should send ERROR when handler throws", async () => {
      const failingHandler = vi.fn(async (): Promise<ServerMessage> => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "FAIL", id: "fail-1" }),
      );
    ...
    it("should send ERROR with generic message when handler throws non-Error", async () => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "FAIL_STR", id: "fail-2" }),
      );
  ...
  describe("constructor", () => {
    it("should accept pre-registered handlers via config", async () => {
      const preHandler = vi.fn(async (_clientId: string, msg: ClientMessage): Promise<ServerMessage> => {
        return { type: "PRE_REPLY", id: msg.id };
      });
      const handlerMap = new Map<string, typeof preHandler>([["PRE", preHandler]]);
      ...
      await preHandlerInstance.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "PRE", id: "pre-1" }),
      );
  ...
  describe("field extraction", () => {
    it("should extract chatId when present", async () => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "ECHO", chatId: "chat-xyz", id: "m1" }),
      );
    ...
    it("should ignore non-string id and chatId", async () => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "ECHO2", id: 123, chatId: 456 }),
      );
    ...
    it("should ignore non-object payload", async () => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "ECHO3", payload: "not-an-object" }),
      );
    ...
    it("should extract array payload as undefined (not an object)", async () => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "ECHO4", payload: [1, 2, 3] }),
      );
    ...
    it("should extract null payload as undefined", async () => {
      ...
      await handler.handleMessage(
        CLIENT_ID,
        JSON.stringify({ type: "ECHO5", payload: null }),
      );
