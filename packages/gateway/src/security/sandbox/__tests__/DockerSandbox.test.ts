...
import { SandboxMode, type DockerConfig, type DockerConstraints, type SandboxExecResult } from '../types.js';

// ---- Helpers ----

const defaultConstraints: DockerConstraints = {
  memory: '512m',
  cpus: '1.0',
  networkNone: true,
  autoRemove: true,
...
function createExecMock() {
  ...
  mock.mockImplementation(async (cmd: string, args: string[]): Promise<{ stdout: string; stderr: string }> => {
...
describe('DockerSandbox', () => {
  describe('constructor', () => {
    ...
    it('accepts custom config', () => {
      const customConfig: DockerConfig = {
        ...defaultConfig,
        image: 'ubuntu:22.04',
        constraints: {
          ...defaultConstraints,
  ...
  describe('exec', () => {
    it('TC-005-3: creates container with correct constraints', async () => {
      ...
      (sandbox as unknown as { execFileAsync: ReturnType<typeof vi.fn> }).execFileAsync = mock;
    ...
    it('TC-005-5: command executed inside container returns result', async () => {
      ...
      const sandbox = new DockerSandbox(defaultConfig);
      (sandbox as unknown as { execFileAsync: typeof execMock }).execFileAsync = execMock;
    ...
    it('handles command failure (non-zero exit code)', async () => {
      const execMock = vi.fn();
      const dockerError = new Error('docker run failed') as Error & { code?: string };
      dockerError.code = '1';
    ...
    it('handles timeout error', async () => {
      const execMock = vi.fn();
      const timeoutError = new Error('Signal: SIGKILL') as Error & { killed?: boolean };
