...
function createMockedManager(overrides?: Partial<import('../types.js').SandboxManagerState>) {
...
describe('SandboxManager', () => {
  describe('detectMode', () => {
    ...
    beforeEach(() => {
      spawnSpy = vi.spyOn(SandboxManager.prototype as never, 'executeCommand');
    });
  ...
  describe('execSandbox', () => {
    let dockerExecSpy: ReturnType<typeof vi.spyOn>;
    ...
    it('executes in Docker mode when Docker is available', async () => {
      const dockerResult: SandboxExecResult = {
        success: true,
