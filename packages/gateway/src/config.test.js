...
import { existsSync, rmSync, readFileSync, statSync, mkdirSync, writeFileSync, } from "node:fs";
import { tmpdir } from "node:os";
import { createDefaultConfig, getOsaiDir, getConfigPath, DEFAULT_CONFIG, 
// New T-006 exports
loadConfig, validateConfig, getConfig, reloadConfig, resetConfigCache, getProviderConfig, getConfigSection, ConfigError, osaiConfigSchema, } from "./config.js";
describe("config module", () => {
    ...
    describe("createDefaultConfig", () => {
        ...
        it("should create config with correct default values from DEFAULT_CONFIG", () => {
            ...
            expect(parsed.agent.circuitBreaker.failureThreshold).toBe(DEFAULT_CONFIG.agent.circuitBreaker.failureThreshold);
            expect(parsed.agent.circuitBreaker.resetTimeoutMs).toBe(DEFAULT_CONFIG.agent.circuitBreaker.resetTimeoutMs);
            expect(parsed.agent.failoverChain).toEqual([...DEFAULT_CONFIG.agent.failoverChain]);
        });
        // ---------------------------------------------------------------
    ...
    describe("DEFAULT_CONFIG", () => {
        ...
        it("should have vector storage configuration", () => {
            ...
            expect(DEFAULT_CONFIG.memory.vectorStorage.qdrant.url).toBe("http://localhost:6333");
        });
    ...
    describe("loadConfig", () => {
        ...
        it("should merge defaults when keys are missing from user config (TT-006-02)", () => {
            ...
            expect(config.agent.circuitBreaker.failureThreshold).toBe(DEFAULT_CONFIG.agent.circuitBreaker.failureThreshold);
            expect(config.agent.circuitBreaker.resetTimeoutMs).toBe(DEFAULT_CONFIG.agent.circuitBreaker.resetTimeoutMs);
        });
        // ---------------------------------------------------------------
    ...
    describe("getConfigSection", () => {
        ...
        const sections = [
            { key: "agent", check: (v) => expect(v.model).toBeDefined() },
            { key: "providers", check: (v) => expect(Object.keys(v).length).toBeGreaterThan(0) },
            { key: "memory", check: (v) => expect(v.rag).toBeDefined() },
            { key: "channels", check: (v) => expect(v.telegram).toBeDefined() },
            { key: "security", check: (v) => expect(v.sandbox).toBeDefined() },
            { key: "skills", check: (v) => expect(v.entries).toBeDefined() },
            { key: "voice", check: (v) => expect(v.stt).toBeDefined() },
        ];
        ...
        it("should return the agent section with correct values", () => {
            const agent = getConfigSection("agent");
            expect(agent.model).toBe(DEFAULT_CONFIG.agent.model);
            expect(agent.failoverChain).toEqual([...DEFAULT_CONFIG.agent.failoverChain]);
            expect(agent.circuitBreaker.failureThreshold).toBe(DEFAULT_CONFIG.agent.circuitBreaker.failureThreshold);
        });
    });
    // ---------------------------------------------------------------
