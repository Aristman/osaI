 ...
 */
function createNChats(chatService, n) {
    const ids = [];
    for (let i = 0; i < n; i++) {
...
describe("ChatArchiveService", () => {
    ...
    describe("enforceActiveLimit / createChatWithLimit", () => {
        it("TT-009-32: should reject creating the 21st active chat", () => {
            ...
            expect(() => archiveService.createChatWithLimit({ name: "Chat 21" })).toThrow(/max.*20.*active.*chat/i);
        });
...
//# sourceMappingURL=ChatArchiveService.test.js.map