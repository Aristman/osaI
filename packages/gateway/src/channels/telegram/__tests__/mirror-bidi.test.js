...
function createMockSender(shouldFail = false, failOnRetry = false) {
    let callCount = 0;
    const callHistory = [];
    const sender = {
        callHistory,
...
function createMockInjector(shouldFail = false) {
    const callHistory = [];
    const injector = {
        callHistory,
...
function createMediaTelegramMessage(overrides = {}) {
    return {
        message_id: 1,
        chat: { id: -1001234567890 },
        from: { id: 12345, first_name: "Test", username: "testuser" },
...
describe("MirrorEngine bidirectional sync (T-008)", () => {
    ...
    it("should support multiple mirrors with different directions", async () => {
        ...
        expect(injector.injectMessage).toHaveBeenCalledWith(expect.objectContaining({ chatId: "chat-beta" }));
        const tgMsgGamma = createTelegramMessage({
            message_id: 101,
            chat: { id: -1003333333333 },
        });
        await multiEngine.receiveMessage(tgMsgGamma);
        expect(injector.injectMessage).toHaveBeenCalledWith(expect.objectContaining({ chatId: "chat-gamma" }));
        ...
        expect(injector.injectMessage).not.toHaveBeenCalledWith(expect.objectContaining({ chatId: "chat-alpha" }));
    });
});
// ---------------------------------------------------------------------------
...
describe("MirrorEngine osaI chat_id binding (T-008)", () => {
    ...
    it("should inject TG messages to the correct osaI chat_id", async () => {
        ...
        expect(injector.injectMessage).toHaveBeenCalledWith(expect.objectContaining({ chatId: "osai-dest-alpha" }));
        ...
        expect(injector.injectMessage).toHaveBeenCalledWith(expect.objectContaining({ chatId: "osai-dest-beta" }));
    ...
    it("should map same TG chat to multiple osaI chats (fan-out)", async () => {
        ...
        expect(injector.injectMessage).toHaveBeenCalledWith(expect.objectContaining({ chatId: "osai-dest-1" }));
        expect(injector.injectMessage).toHaveBeenCalledWith(expect.objectContaining({ chatId: "osai-dest-2" }));
    });
});
// ---------------------------------------------------------------------------
// Mirror loop prevention (dedup by message_id)
...
//# sourceMappingURL=mirror-bidi.test.js.map