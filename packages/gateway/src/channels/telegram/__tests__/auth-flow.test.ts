...
function createTempConfigFile(config: Record<string, unknown> = {}): string {
  const defaultConfig = {
    channels: {
      telegram: {
...
function createAuthFlowConfig(
  overrides: Partial<AuthFlowConfig> = {},
): AuthFlowConfig & { phonePrompt: ReturnType<typeof vi.fn>; codePrompt: ReturnType<typeof vi.fn>; passwordPrompt: ReturnType<typeof vi.fn> } {
  ...
  return {
    ...
    ...overrides,
  } as AuthFlowConfig & { phonePrompt: ReturnType<typeof vi.fn>; codePrompt: ReturnType<typeof vi.fn>; passwordPrompt: ReturnType<typeof vi.fn> };
}

/** Clean up temp files and directories. */
function cleanup(sessionDir: string, configFilePath: string): void {
  try {
...
describe("AuthFlow", () => {
  ...
  describe("2FA password flow", () => {
    it("should prompt for 2FA password when required", async () => {
      ...
      expect(authConfig.passwordPrompt).toHaveBeenCalledWith(
        "Enter 2FA password: ",
        { password: true },
      );
    });
  ...
  describe("error cases", () => {
    ...
    it("should throw after max retries exceeded", async () => {
      ...
      await expect(authFlow.authenticate()).rejects.toThrow(
        "Maximum code verification attempts exceeded",
      );
    });
  });
