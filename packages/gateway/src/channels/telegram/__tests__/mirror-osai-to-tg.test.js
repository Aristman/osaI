...
import { MirrorEngine, MirrorEngineError, markdownToTelegramHtml, } from "../mirror.js";
...
describe("MirrorEngine osai-to-TG message delivery", () => {
    ...
    it("should deliver a plain text message to Telegram", async () => {
        ...
        expect(sender.sendMessage).toHaveBeenCalledWith(-1001234567890, "Hello from osaI");
    });
    it("should convert Markdown to Telegram HTML before sending", async () => {
        ...
        expect(results).toHaveLength(1);
        expect(results[0].success).toBe(true);
        expect(sender.sendMessage).toHaveBeenCalledWith(-1001234567890, "<b>Bold</b> and <i>italic</i>");
    });
...
describe("MirrorEngine on_mirror_message hook", () => {
    ...
    it("should invoke hook for each target in multi-mirror scenario", async () => {
        ...
        expect(hookFn.mock.calls[0][0].telegramChatId).toBe(-1001111111111);
        expect(hookFn.mock.calls[1][0].telegramChatId).toBe(-1002222222222);
    });
});
// ---------------------------------------------------------------------------
...
describe("MirrorEngine error handling and retry", () => {
    it("should retry once on first send failure and succeed on retry", async () => {
        ...
        let callCount = 0;
        const sender = {
            sendMessage: vi.fn(async (_chatId, _text) => {
                callCount++;
                if (callCount === 1) {
    ...
    it("should continue to other targets if one target fails", async () => {
        let callCount = 0;
        const sender = {
            sendMessage: vi.fn(async (chatId, _text) => {
                callCount++;
...
describe("MirrorEngine Markdown formatting preservation", () => {
    it("should preserve bold formatting in mirrored messages", async () => {
        ...
        expect(sender.sendMessage).toHaveBeenCalledWith(-1001234567890, "<b>Important result</b>");
    });
    it("should preserve code blocks in mirrored messages", async () => {
        ...
        const sentText = sender.sendMessage.mock.calls[0][1];
    ...
    it("should preserve links in mirrored messages", async () => {
        ...
        expect(sender.sendMessage).toHaveBeenCalledWith(-1001234567890, 'See <a href="https://example.com">docs</a>');
    });
});
// ---------------------------------------------------------------------------
...
//# sourceMappingURL=mirror-osai-to-tg.test.js.map