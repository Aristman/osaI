...
import type { MirrorConfig, MirrorDirection } from "../../types.js";
...
function createMockSender(
  ...
  callHistory: Array<{ chatId: number; text: string }>;
} {
...
function createMockAgent(
  responseText = "Agent response to user",
  shouldFail = false,
  processingDelayMs = 0,
): GatewayInjector & {
  ...
  const agent: GatewayInjector & {
    injectedMessages: typeof injectedMessages;
  } = {
    injectedMessages,
...
): MediaDownloader & { downloadCallCount: number } {
  let downloadCallCount = 0;

  const downloader: MediaDownloader & { downloadCallCount: number } = {
    downloadCallCount: 0,
...
describe("E2E: Mirror full chain (TG -> Agent -> TG)", () => {
  ...
  let hookCalls: MirrorMessageEvent[];
  ...
  describe("complete roundtrip", () => {
    it("should process TG message through agent and send response", async () => {
      ...
      const responseResults = await engine.mirror(
        "osai-chat-1",
        "The weather is sunny with 25C. Perfect for a walk!",
      );
      ...
      expect(sender.callHistory[0]!.text).toBe(
        "The weather is sunny with 25C. Perfect for a walk!",
      );
    });
    ...
    it("should invoke on_mirror_message hook for both directions", async () => {
      ...
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 200, text: "User question" }),
      );
    ...
    it("should handle multiple messages in sequence", async () => {
      // Message 1
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 1, text: "Hello" }),
      );
      await engine.mirror("osai-chat-1", "Hi there!");

      // Message 2
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 2, text: "How are you?" }),
      );
  ...
  describe("loop prevention", () => {
    ...
    it("should allow different message_ids", async () => {
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 401, text: "First" }),
      );
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 402, text: "Second" }),
      );
  ...
  describe("error handling", () => {
    it("should continue outbound when injection fails", async () => {
      ...
      await errorEngine.receiveMessage(
        createTelegramMessage({ message_id: 700, text: "Will fail" }),
      );
    ...
    it("should continue inbound when sending fails", async () => {
      ...
      await errorEngine.receiveMessage(
        createTelegramMessage({ message_id: 701, text: "Still injects" }),
      );
      expect(agent.injectMessage).toHaveBeenCalledTimes(1);
    });
    ...
    it("should drop messages when engine is not started", async () => {
      ...
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 702 }),
      );
    ...
    it("should handle sender retry on first failure, succeed on second", async () => {
      let callCount = 0;
      const retrySender: TelegramSender & {
        callHistory: Array<{ chatId: number; text: string }>;
      } = {
        callHistory: [],
  ...
  describe("direction modes", () => {
    it("osai-to-tg only: outbound works, inbound ignored", async () => {
      ...
      await dirEngine.receiveMessage(
        createTelegramMessage({ message_id: 800 }),
      );
      expect(agent.injectMessage).toHaveBeenCalledTimes(0);
    });
    ...
    it("tg-to-osai only: inbound works, outbound ignored", async () => {
      ...
      await dirEngine.receiveMessage(
        createTelegramMessage({ message_id: 801, text: "Inbound" }),
      );
      expect(agent.injectMessage).toHaveBeenCalledTimes(1);
    ...
    it("both: full bidirectional", async () => {
      ...
      await engine.receiveMessage(
        createTelegramMessage({ message_id: 802, text: "Bidirectional" }),
      );
      expect(agent.injectMessage).toHaveBeenCalledTimes(1);
  ...
  describe("multiple mirrors", () => {
    it("should route messages to correct osaI chats", async () => {
      ...
      await multiEngine.receiveMessage(
        createTelegramMessage({
          message_id: 900,
