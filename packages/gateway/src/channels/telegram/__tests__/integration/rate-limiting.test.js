...
class RateLimiter {
    ...
    async acquire(fastFail = false) {
        ...
        if (fastFail) {
            throw new RateLimitError(`Rate limit exceeded: ${this.maxRequests} requests per ${this.windowMs}ms`);
        }
...
function createRateLimitedSender(sender, rateLimiter, fastFail = false) {
    return {
...
function createMockSender(delayMs = 0) {
    const callHistory = [];
    return {
        callHistory,
...
describe("Integration: Rate Limiting for Userbot (T-009)", () => {
    ...
    describe("rate-limited sender", () => {
        ...
        it("should block requests exceeding rate limit in fastFail mode", async () => {
            ...
            await expect(rateLimited.sendMessage(-100123, "Blocked")).rejects.toThrow(RateLimitError);
    ...
    describe("realistic Telegram rate limits", () => {
        it("should enforce Telegram Bot API rate limit (~30 msgs/sec)", async () => {
            ...
            await expect(rateLimited.sendMessage(-100123, "Message 31")).rejects.toThrow(RateLimitError);
        });
