...
import type { BridgeResponse, BridgeRequest } from "../types.js";
...
const mockSpawnFactory = vi.hoisted(() => {
  ...
  function incrementSpawnCalls(): void {
    totalSpawnCalls += 1;
  }
  function createMockProcess(): {
    ...
    getStdinData: () => string;
    spawnCalls: Array<{ args: unknown[] }>;
  } {
    const spawnCalls: Array<{ args: unknown[] }> = [];
...
describe("UserbotBridge", () => {
  ...
  describe("lifecycle", () => {
    ...
    it("should pass correct spawn arguments", async () => {
      ...
      const startCall = mock!.spawnCalls.find(
        (c) => c.args[0] === "/usr/bin/python3.11",
      );
  ...
  describe("methods", () => {
    it("sendMessage should send correct request and return result", async () => {
      ...
      const req = JSON.parse(mock.getStdinData().trim()) as BridgeRequest;
    ...
    it("healthCheck should throw on timeout", async () => {
      bridge = new UserbotBridge(
        createBridgeConfig({ healthCheckTimeoutMs: 100 }),
      );
      await bridge.start();
    ...
    it("should throw when calling methods before start", async () => {
      ...
      await expect(bridge.sendMessage("chat-1", "test")).rejects.toThrow(
        UserbotBridgeError,
      );
      await expect(bridge.sendMessage("chat-1", "test")).rejects.toThrow(
        "not running",
  ...
  describe("auto-restart", () => {
    ...
    it("should not exceed maxRestartAttempts", async () => {
      bridge = new UserbotBridge(
        createBridgeConfig({ maxRestartAttempts: 2 }),
      );
      await bridge.start();
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const currentCloseFn = currentMock?.process.on.mock.calls.find(
          (call) => call[0] === "close",
        )?.[1] as ((code: number | null, signal: string | null) => void) | undefined;

        if (currentCloseFn) currentCloseFn(1, null);
        await new Promise((resolve) => setTimeout(resolve, 50));
      }
    ...
    it("should reset restart counter after successful operation", async () => {
      bridge = new UserbotBridge(
        createBridgeConfig({ maxRestartAttempts: 1 }),
      );
      await bridge.start();
      ...
      const req = JSON.parse(lastLine) as BridgeRequest;

      currentMock.emitStdout(
        JSON.stringify({
          type: "health",
          id: req.id,
          data: { status: "ok" },
      ...
      const newCloseFn = newProcess.on.mock.calls.find(
        (call) => call[0] === "close",
      )?.[1] as ((code: number | null, signal: string | null) => void) | undefined;

      // Second crash should still trigger restart (counter was reset)
      if (newCloseFn) newCloseFn(1, null);
      await new Promise((resolve) => setTimeout(resolve, 50));
  ...
  describe("invalid JSON handling", () => {
    it("should log invalid JSON from Python without crashing", async () => {
      ...
      const logArg = localWarnSpy.mock.calls[0]?.[0] as Record<string, unknown>;
