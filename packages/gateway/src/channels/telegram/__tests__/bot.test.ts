...
import { TelegramBot, TelegramBotError, type TelegramBotOptions } from "../bot.js";
import type { TelegramBotConfig } from "../types.js";
import type { ChannelContext, ChannelResult } from "../../types.js";
 ...
 */
function createMockBotApi(overrides: Record<string, unknown> = {}) {
  ...
  return {
    use: vi.fn().mockImplementation((_middleware: unknown) => {
      middlewareStack.push({ type: "middleware", filter: undefined, handler: _middleware });
    }),
    command: vi.fn().mockImplementation((_command: string, _handler: unknown) => {
      middlewareStack.push({ type: "command", filter: _command, handler: _handler });
    }),
    on: vi.fn().mockImplementation((_filter: string, _handler: unknown) => {
      middlewareStack.push({ type: "on", filter: _filter, handler: _handler });
    }),
    start: vi.fn().mockImplementation((_opts?: Record<string, unknown>) => {
      if (overrides.startFn) {
        return (overrides.startFn as () => Promise<void>)();
      }
...
function createTestBotConfig(overrides: Partial<TelegramBotConfig> = {}): TelegramBotConfig {
  return {
    enabled: true,
...
function createBotOptions(
  ...
  optionsOverrides: Partial<Omit<TelegramBotOptions, "config">> = {},
): TelegramBotOptions {
  return {
 ...
 */
function createBotWithMock(
  options: TelegramBotOptions,
): { bot: TelegramBot; mockApi: ReturnType<typeof createMockBotApi> } {
...
describe("TelegramBot", () => {
  ...
  describe("construction", () => {
    ...
    it("should store allowedUsers from config", () => {
      const bot = new TelegramBot(
        createBotOptions({ allowedUsers: ["111", "222", "333"] }),
      );
  ...
  describe("ChannelHandler interface", () => {
    ...
    it("onMessage should handle permission_request payload", async () => {
      ...
      const bot = new TelegramBot(
        createBotOptions({}, { onPermissionRequest: permissionCallback }),
      );
      ...
      expect(permissionCallback).toHaveBeenCalledWith(
        expect.objectContaining({ tool: "shell", action: "exec" }),
      );
    ...
    it("onMessage should handle notification payload", async () => {
      ...
      const bot = new TelegramBot(
        createBotOptions({}, { onNotification: notificationCallback }),
      );
  ...
  describe("allowedUsers whitelist", () => {
    ...
    it("should block users not in the whitelist", async () => {
      ...
      const bot = new TelegramBot(
        createBotOptions({ allowedUsers: ["99999"] }),
      );
    ...
    it("should handle single allowed user", () => {
      const bot = new TelegramBot(
        createBotOptions({ allowedUsers: ["111"] }),
      );
  ...
  describe("graceful degradation", () => {
    ...
    it("should handle permission_request callback throwing", async () => {
      ...
      const bot = new TelegramBot(
        createBotOptions({}, { onPermissionRequest: errorCallback }),
      );
    ...
    it("should handle notification callback throwing", async () => {
      const errorCallback = vi.fn().mockRejectedValue(new Error("Callback failed"));
      const bot = new TelegramBot(
        createBotOptions({}, { onNotification: errorCallback }),
      );
