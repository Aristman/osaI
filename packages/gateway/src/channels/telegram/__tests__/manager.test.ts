...
describe("TelegramManager", () => {
  ...
  describe("construction", () => {
    ...
    it("should default to bot-only mode when userbot is disabled", () => {
      const manager = new TelegramManager(
        createManagerConfig({ userbot: { enabled: false, apiId: 1, apiHash: "x", phone: "1" } }),
      );
  ...
  describe("start", () => {
    ...
    it("should not start bot when bot is disabled", async () => {
      const manager = new TelegramManager(
        createManagerConfig({ bot: { enabled: false, token: "x", allowedUsers: [] } }),
      );
    ...
    it("should start mirror engine when mirrors are configured", async () => {
      const mirrors: readonly MirrorConfig[] = [
        { chatId: "chat-1", telegramChatId: -1001234567890, direction: "both" as MirrorDirection },
      ];

      const manager = new TelegramManager(
        createManagerConfig({ mirrors }),
      );
  ...
  describe("stop", () => {
    ...
    it("should stop all running components in bot+userbot mode with mirrors", async () => {
      const mirrors: readonly MirrorConfig[] = [
        { chatId: "c1", telegramChatId: -100111, direction: "both" as MirrorDirection },
      ];
  ...
  describe("status", () => {
    it("should return complete status object", async () => {
      const manager = new TelegramManager(createManagerConfig());
      const status: TelegramManagerStatus = manager.getStatus();
  ...
  describe("type exports", () => {
    ...
    it("BridgeRequest should accept all request types", () => {
      const types: Array<BridgeRequest["type"]> = [
        "auth",
        "send_message",
        "listen",
        "get_chats",
      ...
      for (const type of types) {
        const request: BridgeRequest = { type, id: `test-${type}`, params: {} };
    ...
    it("BridgeResponse should accept all response types", () => {
      const types: Array<BridgeResponse["type"]> = [
        "auth_result",
        "message",
        "send_result",
        "error",
      ...
      for (const type of types) {
        const response: BridgeResponse = { type, id: `test-${type}`, data: null };
        expect(response.type).toBe(type);
      }
    });

    it("MirrorDirection should accept valid values", () => {
      const directions: MirrorDirection[] = ["both", "osai-to-tg", "tg-to-osai"];
    ...
    it("TelegramManagerMode should accept valid values", () => {
      const modes: TelegramManagerMode[] = ["bot-only", "bot+userbot"];
