...
function createMockHandler(channelName: string): {
  ...
  const onMessage = vi.fn(async (_ctx: ChannelContext): Promise<ChannelResult> => ({
    payload: { _mock: true, channel: channelName },
  }));
  const send = vi.fn(async () => {});
  const subscribe = vi.fn((_event: string, _cb: (data: Record<string, unknown>) => void) => {});
...
function createFailingHandler(channelName: string): ChannelHandler {
  return {
    channelName,
    onMessage: async () => {
...
describe("ChannelRouter", () => {
  ...
  describe("handler registration", () => {
    ...
    it("should throw when registering a handler with a duplicate channel name", () => {
      ...
      expect(() => router.register(cli2)).toThrow(
        'Channel handler already registered: "cli"',
      );
    });
  ...
  describe("handler unregistration", () => {
    ...
    it("should throw when unregistering a non-existent channel", async () => {
      await expect(router.unregister("nonexistent")).rejects.toThrow(
        'No channel handler registered: "nonexistent"',
      );
    });
  });
  ...
  describe("message dispatch", () => {
    ...
    it("should return void when handler returns void", async () => {
      const voidHandler: ChannelHandler = {
        channelName: "void-ch",
        onMessage: async () => {
          // No return value
        },
  ...
  describe("unknown channel handling", () => {
    it("should throw ChannelHandlerError for unknown channel", async () => {
      ...
      await expect(router.dispatch("unknown_channel", ctx)).rejects.toThrow(
        ChannelHandlerError,
      );
    });
    ...
    it("should include channel name in error", async () => {
      ...
      try {
        ...
        const chErr = err as ChannelHandlerError;
  ...
  describe("dispatch result delivery", () => {
    ...
    it("should preserve result metadata when provided by handler", async () => {
      const metaHandler: ChannelHandler = {
        channelName: "meta-ch",
        onMessage: async () => ({
          payload: { data: "test" },
  ...
  describe("handler error handling", () => {
    it("should wrap handler errors in ChannelHandlerError", async () => {
      ...
      await expect(router.dispatch("broken", createContext())).rejects.toThrow(
        ChannelHandlerError,
      );
    });
    ...
    it("should pass through ChannelHandlerError without re-wrapping", async () => {
      ...
      const errorThrower: ChannelHandler = {
        channelName: "test-ch",
  ...
  describe("destroy", () => {
    ...
    it("should handle handler destroy errors gracefully", async () => {
      const errorDestroyer: ChannelHandler = {
