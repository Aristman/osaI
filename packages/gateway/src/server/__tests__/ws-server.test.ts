...
describe("WsServer", () => {
  ...
  describe("connection events", () => {
    ...
    it("should fire onDisconnection callback", async () => {
      ...
      const disconnected = vi.fn<[string, number, Buffer]>();
  ...
  describe("messaging", () => {
    it("should send a message to a specific client", async () => {
      ...
      const connected = vi.fn<[string, WebSocket]>((id) => {
        receivedClientId = id;
      });
      server.onConnection(connected);
      ...
      const msg = (await received) as string;
    ...
    it("should broadcast a message to all connected clients", async () => {
      ...
      expect(JSON.parse(r1 as string)).toEqual({ type: "announcement", text: "hi" });
      expect(JSON.parse(r2 as string)).toEqual({ type: "announcement", text: "hi" });
  ...
  describe("logging", () => {
    ...
    function createLogSink(): {
      ...
      const stream = new Writable({
        write(chunk, _encoding, cb) {
          ...
          entries.push(JSON.parse(str.trim()) as Record<string, unknown>);
          cb();
        },
      });
    ...
    it("should log connection events", async () => {
      ...
      const connectLogs = entries.filter(
        (e) => e.msg === "Client connected",
      );
    ...
    it("should log disconnection events", async () => {
      ...
      const disconnectLogs = entries.filter(
        (e) => e.msg === "Client disconnected",
      );
