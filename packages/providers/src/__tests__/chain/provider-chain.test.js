...
import { ProviderError, ProviderUnavailableError, RateLimitError, } from '../../errors.js';
...
describe('ProviderChain', () => {
    ...
    describe('TT-002-74: rate limit auth rotation', () => {
        it('should fall back to next provider when current returns rate limit', async () => {
            ...
            zAiComplete.mockRejectedValue(new RateLimitError('Rate limit', 'z-ai', { retryAfterMs: 1000 }));
            yandexComplete.mockResolvedValue(createSuccessResponse('yandex'));
            const chain = new ProviderChain([zAi, yandex], { logger, authKeys: { 'z-ai': ['key1', 'key2'] } });
            const response = await chain.execute(defaultRequest);
