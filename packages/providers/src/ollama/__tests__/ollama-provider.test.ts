...
import type { LLMRequest, ChatMessage } from '../../types.js';
...
const SIMPLE_MESSAGES: readonly ChatMessage[] = [
...
describe('OllamaProvider', () => {
  ...
  describe('isAvailable', () => {
    ...
    it('should return false when /api/tags returns 500', async () => {
      mockFetch.mockResolvedValue(
        createJsonResponse({ error: 'internal server error' }, 500),
      );
  ...
  describe('complete', () => {
    ...
    it('should use default model when request.model is empty', async () => {
      ...
      const requestWithNoModel: LLMRequest = {
        model: '',
        messages: SIMPLE_MESSAGES,
      };
    ...
    it('should throw ProviderError on non-success HTTP status', async () => {
      mockFetch.mockResolvedValue(
        createJsonResponse({ error: 'model not found' }, 404),
      );
  ...
  describe('stream', () => {
    ...
    it('should include usage in final chunk when provided by Ollama', async () => {
      ...
      const lastChunk = chunks[0] as { usage?: { promptTokens: number; completionTokens: number; totalTokens: number } };
  ...
  describe('getStatus', () => {
    ...
    it('should return Available after successful isAvailable', async () => {
      mockFetch.mockResolvedValue(
        createJsonResponse({ model_names: ['llama3'], models: [] }),
      );
  ...
  describe('error mapping', () => {
    ...
    it('should include Ollama error message in ProviderError', async () => {
      ...
      mockFetch.mockResolvedValue(
        createJsonResponse(ollamaError, 404),
      );
