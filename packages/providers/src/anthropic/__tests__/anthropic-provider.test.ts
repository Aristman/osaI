...
import type { ProviderConfig, LLMRequest, ToolDefinition } from '../../types.js';
...
function createProvider(config: ProviderConfig = TEST_CONFIG): AnthropicProvider {
...
describe('AnthropicProvider', () => {
  ...
  describe('isAvailable', () => {
    ...
    it('should return true when rate limited (429)', async () => {
      mockCountTokens.mockRejectedValue(
        createAPIError('Rate limited', 429),
      );
    ...
    it('should return true for other 4xx errors (API reachable)', async () => {
      mockCountTokens.mockRejectedValue(
        createAPIError('Bad request', 400),
      );
    ...
    it('should return false on connection error', async () => {
      mockCountTokens.mockRejectedValue(
        createConnectionError('Connection refused'),
      );
  ...
  describe('complete', () => {
    ...
    it('should map RateLimitError (429)', async () => {
      mockCreate.mockRejectedValue(
        createAPIError('Rate limited', 429, { 'retry-after': '5' }),
      );
    ...
    it('should map auth errors (401/403) to AuthError', async () => {
      mockCreate.mockRejectedValue(
        createAPIError('Invalid API key', 401),
      );
    ...
    it('should map connection errors to ProviderUnavailableError', async () => {
      mockCreate.mockRejectedValue(
        createConnectionError('ECONNREFUSED'),
      );
    ...
    it('should extract retry-after from RateLimitError headers', async () => {
      mockCreate.mockRejectedValue(
        createAPIError('Rate limited', 429, { 'retry-after': '10' }),
      );
      ...
      try {
        ...
        expect((error as RateLimitError).retryAfterMs).toBe(10_000);
      }
    });
  });
  ...
  describe('stream', () => {
    ...
    it('should accumulate tool input JSON from input_json_delta events', async () => {
      ...
      const provider = createProvider();
      const chunks: Array<{ content: string; toolCalls?: readonly { id: string; name: string; arguments: string }[]; finishReason?: string }> = [];
    ...
    it('should yield chunks with correct provider and model', async () => {
      ...
      const provider = createProvider();
      const chunks: Array<{ provider: string; model: string }> = [];
    ...
    it('should map stream errors to ProviderError hierarchy', async () => {
      mockCreate.mockRejectedValue(
        createAPIError('Server error', 500),
      );
