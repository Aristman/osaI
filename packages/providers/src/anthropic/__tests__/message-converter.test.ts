...
describe('toAnthropicMessages', () => {
  ...
  it('should convert assistant tool_calls to tool_use content blocks', () => {
    ...
    const content = result[1]?.content as Array<{ type: string; text?: string; id?: string; name?: string; input?: unknown }>;
  ...
  it('should handle assistant with only tool calls (no text)', () => {
    ...
    const result = toAnthropicMessages(messages);
    const content = result[1]?.content as Array<{ type: string }>;
  ...
  it('should handle multiple tool calls from assistant', () => {
    ...
    const result = toAnthropicMessages(messages);
    const content = result[1]?.content as Array<{ type: string; name: string }>;
    expect(content).toHaveLength(2);
  ...
  it('should convert tool result messages to tool_result content blocks', () => {
    ...
    const content = toolMsg?.content as Array<{ type: string; tool_use_id: string; content: string }>;
