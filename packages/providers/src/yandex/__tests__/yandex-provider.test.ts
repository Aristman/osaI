...
import type { ProviderConfig, LLMRequest } from '../../types.js';
...
describe('YandexProvider', () => {
  ...
  describe('complete', () => {
    ...
    it('should throw ProviderUnavailableError on 500 response', async () => {
      ...
      const provider = createProvider();
      await expect(provider.complete(basicRequest)).rejects.toThrow(
        ProviderUnavailableError,
      );
    });
  ...
  describe('stream', () => {
    ...
    it('should include usage in final chunk', async () => {
      ...
      const provider = createProvider();
      let lastChunk: { usage?: unknown } | undefined;
    ...
    it('should yield chunks with correct provider and model', async () => {
      ...
      const provider = createProvider();
      const collected: Array<{ provider: string; model: string }> = [];
    ...
    it('should handle TRUNCATED status in streaming', async () => {
      ...
      const provider = createProvider();
      const collected: Array<{ finishReason?: string }> = [];
