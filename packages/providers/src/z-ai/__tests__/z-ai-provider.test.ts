...
const { mockList, mockCreate } = vi.hoisted(() => {
  return {
    mockList: vi.fn<() => Promise<unknown>>(),
...
import type { LLMRequest, ToolDefinition } from '../../types.js';
...
function createProvider(
  config?: Partial<import('../../types.js').ProviderConfig>,
): ZAiProvider {
...
describe('ZAiProvider', () => {
  ...
  describe('complete', () => {
    ...
    it('should pass tools in OpenAI format', async () => {
      ...
      const request: LLMRequest = { ...basicRequest, tools };
      await provider.complete(request);
  ...
  describe('complete -- error mapping', () => {
    it('TT-002-14: should throw RateLimitError on 429 response', async () => {
      const apiError = new OpenAI_APIError(
        429,
    ...
    it('TT-002-15: should throw ProviderUnavailableError on 5xx response', async () => {
      ...
      try {
        ...
        const ue = error as ProviderUnavailableError;
    ...
    it('should throw ProviderError for unknown error types', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(ProviderError);
        expect((error as ProviderError).providerId).toBe('z-ai');
      }
    });
  ...
  describe('stream', () => {
    it('TT-002-12: should yield LLMChunk objects with content deltas', async () => {
      ...
      expect((results[0] as { provider: string }).provider).toBe('z-ai');
      expect((results[0] as { model: string }).model).toBe('glm-5');
      expect((results[2] as { finishReason: string }).finishReason).toBe(
        'stop',
      );
    });
    ...
    it('should include usage when present in stream chunk', async () => {
      ...
      expect((results[0] as { usage: unknown }).usage).toEqual({
        promptTokens: 15,
        completionTokens: 5,
        totalTokens: 20,
      });
    ...
    it('should map tool_calls from stream deltas', async () => {
      ...
      expect(
        (results[0] as { toolCalls?: unknown[] }).toolCalls,
      ).toBeDefined();
      expect(
        (results[0] as { toolCalls: unknown[] }).toolCalls,
      ).toHaveLength(1);
      expect(
        (results[0] as { toolCalls: unknown[] }).toolCalls[0],
      ).toEqual({
        id: 'call_stream_1',
        name: 'search',
        arguments: '',
      ...
      expect(
        (results[1] as { toolCalls?: unknown[] }).toolCalls,
      ).toBeDefined();
      expect(
        (results[1] as { toolCalls: unknown[] }).toolCalls[0],
      ).toEqual({
        id: '',
        name: '',
        arguments: '{"q":"test"}',
