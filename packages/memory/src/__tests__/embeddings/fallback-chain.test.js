...
// Mock helpers
// ---------------------------------------------------------------------------
function makeProvider(name, available, embedFn) {
    const singleResult = {
...
describe('EmbeddingFallbackChain', () => {
    ...
    it('TC-007: chain throws Error when all providers are unavailable', async () => {
        ...
        await expect(chain.embed('hello')).rejects.toThrow(/no.*available|all.*unavailable/i);
    });
    // -----------------------------------------------------------------------
...
//# sourceMappingURL=fallback-chain.test.js.map