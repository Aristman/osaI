...
function makeEmbedResponse(embedding: number[]): Response {
...
describe('OllamaEmbeddingProvider', () => {
  ...
  it('TC-001: embed("hello") returns EmbeddingResult with vector length 768', async () => {
    ...
    expect(url).toBe(
      `${EMBEDDING_DEFAULTS.ollamaEndpoint}/api/embeddings`,
    );
  ...
  it('embed(texts) batches multiple texts', async () => {
    ...
    fetchSpy.mockImplementation(() =>
      Promise.resolve(makeEmbedResponse(make768())),
    );
