...
function createMockEmbeddingProvider() {
    return {
        name: 'mock-embedder',
        embed: vi.fn().mockImplementation(async (textOrTexts) => {
...
// ---------------------------------------------------------------------------
function createMockLLMProvider(facts) {
    const responseContent = JSON.stringify(facts);
    return {
        id: 'mock-llm',
        name: 'Mock LLM',
        isAvailable: vi.fn().mockResolvedValue(true),
        complete: vi.fn().mockImplementation(async (_request) => ({
...
function createTestEntry(content, overrides = {}) {
...
describe('Memory System E2E', () => {
    ...
    describe('TC-006: E2E remember -> recall through RAG pipeline', () => {
        it('should remember a fact and recall it via chat', async () => {
            ...
            const stored = await memoryManager.remember('The user works at a tech company in Moscow', 'chat-abc');
        ...
        it('should combine chat-scoped and long-term memories in recall', async () => {
            ...
            const chatEntry = createTestEntry('User asked about weather today', { tier: MemoryTier.Chat, chatId: 'chat-xyz' });
    ...
    describe('TC-007: E2E buildContext with pruning on overflow', () => {
        ...
        it('should preserve system prompt even when pruning', async () => {
            ...
            const systemEntries = result.context.filter((e) => e.priority === PruningPriority.SystemPrompt);
...
describe('MemoryService Facade E2E', () => {
    ...
    it('should initialize, store, query, and destroy', async () => {
        ...
        expect(results.length).toBeGreaterThanOrEqual(1);
        // Build context
        const context = service.buildContext([{ role: 'user', content: 'Hello', priority: PruningPriority.EarlyHistory, tokenCount: 5 }], 'System prompt', 10000);
    ...
    it('should throw on operations before init', async () => {
        ...
        await expect(service.store({
            ...
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
        }, MemoryTier.LongTerm)).rejects.toThrow('not initialized');
...
//# sourceMappingURL=memory-e2e.test.js.map