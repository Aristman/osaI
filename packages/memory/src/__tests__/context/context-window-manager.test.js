...
import { PruningPriority, } from '../../types/context.js';
...
function makeEntry(role, content, priority) {
    return {
        role,
        content,
        priority,
...
describe('pruneByPriority', () => {
    it('TC-003: сначала удаляет LT RAG results, потом KB chunks', () => {
        ...
        const ragEntry = makeRAGEntry('Long-term memory context: ' + 'x'.repeat(100));
        const kbEntry = makeKBEntry('Knowledge base chunk: ' + 'x'.repeat(100));
        ...
        const maxTokens = systemPrompt.tokenCount +
            historyMsg.tokenCount +
            assistantMsg.tokenCount +
            2; // minimal buffer -- RAG and KB won't fit
        const result = pruneByPriority(messages, maxTokens, 
    ...
    it('TC-005: reservedForResponse=1024 резервирует токены для ответа', () => {
        ...
        const messages = [systemPrompt, historyMsg];
...
describe('ContextWindowManager', () => {
    ...
    let stubSummarizer;
    beforeEach(() => {
        stubSummarizer = createStubSummarizer('Previous conversation summary.');
        manager = new ContextWindowManager({
            summarizer: stubSummarizer,
        });
    });
    it('TC-001: buildContext() оставляет system prompt без изменений', () => {
        const systemPrompt = 'You are a helpful assistant. Follow all instructions carefully.';
        const messages = [
            makeHistoryMessage('Hello'),
            makeAssistantMessage('Hi there!'),
        ];
        const result = manager.buildContext(messages, systemPrompt, 8192);
        // System prompt entry must be present and unchanged
        const systemEntry = result.context.find((e) => e.priority === PruningPriority.SystemPrompt);
    ...
    it('TC-002: buildContext() при превышении 80% threshold запускает summarization', async () => {
        ...
        const result = customManager.buildContext(bigMessages, systemPrompt, maxTokens);
