...
import { PruningPriority, CONTEXT_DEFAULTS, } from '../types/context.js';
import { VECTOR_STORAGE_DEFAULTS, } from '../types/vector-storage.js';
// ---------------------------------------------------------------------------
// MemoryCategory enum
// ---------------------------------------------------------------------------
...
describe('RAGQuery', () => {
    it('accepts a minimal query with only text', () => {
        const query = { text: 'What are user preferences?' };
...
describe('PruningPriority', () => {
    ...
    it('has exactly 5 named values', () => {
        ...
        const namedValues = Object.values(PruningPriority).filter((v) => typeof v === 'number');
