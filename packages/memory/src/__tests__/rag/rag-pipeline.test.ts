...
function createMockVectorStorage(
  options?: Partial<{ searchResults: Array<{ id: string; score: number; metadata: Record<string, string | number | boolean> }>; searchError: Error }>,
): VectorStorage {
...
const testConfig: RAGConfig = { ...RAG_DEFAULTS };
  ...
  describe('TC-002: query calls vector search with obtained vector', () => {
    it('should call search() with the embedding vector, topK, and minSimilarity', async () => {
      ...
      expect(storage.search).toHaveBeenCalledWith(
        expectedVector,
        testConfig.defaultTopK,
        testConfig.defaultMinSimilarity,
      );
    });
