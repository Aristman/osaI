...
// Mock Factories
// =================================================================
function createMockRAGPipeline(options) {
    return {
...
function createTestRAGResult(overrides) {
    return {
...
describe('MemoryManager (T-006)', () => {
    ...
    describe('TC-001: store() saves to long-term tier with embedding', () => {
        ...
        it('should upsert vector in vector storage for long-term tier', async () => {
            ...
            expect(vectorStorage.upsert).toHaveBeenCalledWith(entry.id, expectedVector, expect.objectContaining({
        ...
        it('should store entry in repository for long-term tier', async () => {
            ...
            expect(repository.store).toHaveBeenCalledWith(expect.objectContaining({
                id: 'mem-lt-1',
                content: 'Important fact to remember',
                tier: 'long-term',
            }));
    ...
    describe('TC-002: remember() creates long-term memory', () => {
        it('should create a MemoryEntry with tier=long-term', async () => {
            ...
            expect(repository.store).toHaveBeenCalledOnce();
            const stored = vi.mocked(repository.store).mock.calls[0][0];
        ...
        it('should generate unique id for each remembered content', async () => {
            ...
            const firstId = vi.mocked(repository.store).mock.calls[0][0];
            vi.mocked(repository.store).mockClear();
            await manager.remember('Second memory');
            const secondId = vi.mocked(repository.store).mock.calls[0][0];
    ...
    describe('TC-007: store() logs actions with trace_id', () => {
        it('should log store action with trace_id when trace context is set', async () => {
            ...
            const storeCall = calls.find((call) => typeof call[0] === 'object' && call[0] !== null && call[0].action === 'store');
            expect(storeCall).toBeDefined();
            expect(storeCall[0]).toHaveProperty('trace_id');
        });
        it('should include trace_id in log entries from TraceContext', async () => {
            ...
            const storeCall = calls.find((call) => typeof call[0] === 'object' && call[0] !== null && call[0].action === 'store');
            expect(storeCall).toBeDefined();
            expect(storeCall[0].trace_id).toBe('test-trace-123');
            // Reset
        ...
        it('should log forget action with trace_id and memoryId', async () => {
            ...
            const calls = vi.mocked(loggerMock.info).mock.calls;
            const forgetCall = calls.find((call) => typeof call[0] === 'object' && call[0] !== null && call[0].action === 'forget');
            expect(forgetCall).toBeDefined();
            expect(forgetCall[0].memoryId).toBe('mem-to-forget');
        });
        it('should log query action with trace_id', async () => {
            ...
            const calls = vi.mocked(loggerMock.info).mock.calls;
            const queryCall = calls.find((call) => typeof call[0] === 'object' && call[0] !== null && call[0].action === 'query');
        ...
        it('should log recall action with chatId and trace_id', async () => {
            ...
            const calls = vi.mocked(loggerMock.info).mock.calls;
            const recallCall = calls.find((call) => typeof call[0] === 'object' && call[0] !== null && call[0].action === 'recall');
            expect(recallCall).toBeDefined();
            expect(recallCall[0].chatId).toBe('chat-xyz');
        });
        it('should log error with trace_id on failure', async () => {
            ...
            const errorCall = errorCalls.find((call) => typeof call[0] === 'object' && call[0] !== null && call[0].action === 'store_error');
