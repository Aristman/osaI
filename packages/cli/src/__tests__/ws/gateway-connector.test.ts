...
describe("executeGatewayCommand", () => {
  it("should return success with response data when Gateway responds", async () => {
    ...
    try {
      ...
      expect((result.data as Record<string, unknown>).type).toBe("command_response");
    } finally {
      wsServer.close();
      server.close();
    }
  ...
  it("should send command with correct payload", async () => {
    ...
    wsServer.on("connection", (ws) => {
      ws.on("message", (data) => {
        const msg = JSON.parse(data.toString()) as Record<string, unknown>;
        if (msg.type === "command") {
          receivedCommand = msg.payload.command as string;
          receivedArgs = msg.payload.args as Record<string, unknown> | undefined;
