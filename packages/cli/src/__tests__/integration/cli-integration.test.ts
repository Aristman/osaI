...
function waitForClient(wsServer: InstanceType<typeof WebSocketServer>): Promise<InstanceType<typeof WebSocket>> {
  return new Promise((resolve) => {
    wsServer.on("connection", (ws) => resolve(ws));
  });
}

function closeMockGateway(gw: MockGateway): void {
  try {
    gw.wsServer.close();
  } catch {
    // ignore
...
describe("CLI Integration Tests", () => {
  ...
  describe("TT-006-07: full cycle -- connect -> message -> response", () => {
    it("should complete full cycle: connect, subscribe, send message, receive text block", async () => {
      ...
      try {
        ...
        const subscribeMsg = receivedMessages.find(
          (m) => (m as Record<string, unknown>)["type"] === "subscribe",
        );
        expect(subscribeMsg).toBeDefined();
        expect((subscribeMsg as Record<string, unknown>)["session_id"]).toBe("test-session-e2e");
        ...
        const userMsg = receivedMessages.find(
          (m) => (m as Record<string, unknown>)["type"] === "message",
        );
        expect(userMsg).toBeDefined();
        expect(((userMsg as Record<string, unknown>)["payload"] as Record<string, unknown>)["content"]).toBe(
          "Hello, osaI!",
        );

        // Step 5: Server sends response (text block)
        const blockPromise = new Promise<BlockStreamMessage>((resolve) => {
    ...
    it("should handle permission request -> auto-approve -> response cycle", async () => {
      ...
      try {
        ...
        const permPromise = new Promise<PermissionRequestMessage>((resolve) => {
        ...
        const permResp = receivedMessages.find(
          (m) => (m as Record<string, unknown>)["type"] === "permission_response",
        );
        expect(permResp).toBeDefined();
        expect((permResp as Record<string, unknown>)["request_id"]).toBe("perm-e2e-001");
        expect(((permResp as Record<string, unknown>)["payload"] as Record<string, unknown>)["allow"]).toBe(true);
    ...
    it("should handle tool_stream -> block sequence (streaming response)", async () => {
      ...
      try {
        ...
        const toolStreamMessages: ToolStreamMessage[] = [];
        router.on("tool_stream", (msg) => {
          toolStreamMessages.push(msg);
        });

        // Collect block messages
        const blockMessages: BlockStreamMessage[] = [];
        router.on("block", (msg) => {
  ...
  describe("error handling", () => {
    it("should handle Gateway sending invalid JSON gracefully", async () => {
      const gw = await startMockGateway();
      const errors: Array<{ raw: string; error: Error }> = [];
      let serverWs: InstanceType<typeof WebSocket> | null = null;
    ...
    it("should handle connection dropping during message exchange", async () => {
      ...
      try {
        ...
        const disconnectedFired = new Promise<boolean>((resolve) => {
          const client = new GatewayClient({
            url: gw.url,
            logger: createTestLogger(),
            maxRetries: 0,
  ...
  describe("permission protocol E2E", () => {
    it("should send correct permission_response format for allow", async () => {
      ...
      try {
        ...
        expect(permResp).toBeDefined();
        const msg = permResp as Record<string, unknown>;
        expect(msg["session_id"]).toBe("session-001");
        expect(msg["request_id"]).toBe("req-001");
        expect((msg["payload"] as Record<string, unknown>)["allow"]).toBe(true);
    ...
    it("should send correct permission_response format for deny", async () => {
      ...
      try {
        ...
        expect(permResp).toBeDefined();
        const msg = permResp as Record<string, unknown>;
        expect((msg["payload"] as Record<string, unknown>)["allow"]).toBe(false);
