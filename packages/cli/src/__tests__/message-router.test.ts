...
function createMockClient() {
  ...
  } as unknown as GatewayClient & { simulateMessage: (data: string) => void };
...
describe("MessageRouter", () => {
  ...
  it("TT-002-04: should route tool_stream messages and provide tool, action, chunk", () => {
    ...
    const received: ToolStreamMessage[] = [];
    router.on("tool_stream", (msg) => {
      received.push(msg);
    });

    const toolStreamMsg: ToolStreamMessage = {
      type: "tool_stream",
      session_id: "sess-1",
      chat_id: "chat-1",
      tool: "filesystem",
  ...
  it("TT-002-05: should route block messages and provide block_type, content", () => {
    ...
    const received: BlockStreamMessage[] = [];
    router.on("block", (msg) => {
      received.push(msg);
    });

    const blockMsg: BlockStreamMessage = {
      type: "block",
      session_id: "sess-1",
      chat_id: "chat-1",
      block_type: "code",
  ...
  it("TT-002-06: should route permission_request messages and provide request_id, risk_level", () => {
    ...
    const received: PermissionRequestMessage[] = [];
  ...
  it("should warn when attaching to a client while already attached", () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      "MessageRouter is already attached to a client",
    );
