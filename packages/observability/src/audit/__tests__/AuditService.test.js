...
import { AuditService, AuditEventType, } from "../types.js";
...
describe("AuditService (T-007)", () => {
    ...
    describe("validation", () => {
        it("should throw if action is missing", () => {
            expect(() => service.log({})).toThrow("Audit entry validation failed: 'action' is required");
        });
        it("should throw if action is null", () => {
            expect(() => service.log({ action: null })).toThrow("Audit entry validation failed: 'action' is required");
        });
    });
    // -------------------------------------------------------------------------
    // Additional: cleanup
