...
function createMockChunk(overrides: Partial<KnowledgeChunk> = {}): KnowledgeChunk {
  return {
    id: 'chunk_doc-001_0',
    documentId: 'doc-001',
...
function createMockRepository(documents: KnowledgeDocument[] = [], chunks: KnowledgeChunk[] = []): {
  repository: KnowledgeRepository;
  getDocument: ReturnType<typeof vi.fn>;
  listDocuments: ReturnType<typeof vi.fn>;
  getChunksByDocument: ReturnType<typeof vi.fn>;
...
describe('SourceManager', () => {
  ...
  let pipeline: ReturnType<typeof createMockIngestPipeline>;
  let repo: ReturnType<typeof createMockRepository>;
  let vector: ReturnType<typeof createMockVectorStorage>;
  ...
  describe('addSource', () => {
    it('TC-006-1: should ingest document and return document ID', async () => {
      ...
      expect(pipeline.ingestDocument).toHaveBeenCalledWith(
        '/path/to/file.txt',
        expect.any(Buffer),
        undefined,
        undefined,
  ...
  describe('removeSource (not found)', () => {
    it('TC-006-3: should throw SourceNotFoundError for non-existent document', async () => {
      ...
      await expect(manager.removeSource('doc-nonexistent')).rejects.toThrow(
        'Source not found: doc-nonexistent',
      );
  ...
  describe('addSource (duplicate)', () => {
    it('TC-006-7: should propagate DocumentAlreadyExistsError from pipeline', async () => {
      ...
      pipeline.ingestDocument.mockRejectedValue(
        new DocumentAlreadyExistsError('/path/to/existing.txt'),
      );

      await expect(
        manager.addSource('/path/to/existing.txt', Buffer.from('content')),
      ).rejects.toThrow(DocumentAlreadyExistsError);

      await expect(
        manager.addSource('/path/to/existing.txt', Buffer.from('content')),
      ).rejects.toThrow('Document already exists: /path/to/existing.txt');
    });
  });
});
