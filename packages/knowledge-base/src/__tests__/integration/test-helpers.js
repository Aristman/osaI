...
export function createTestEnvironment() {
    ...
    const ingestPipeline = new IngestPipeline(repository, parserRegistry, embeddingProvider, vectorStorage, db);
    // Real KB search
    const kbSearch = new KBSearch(embeddingProvider, vectorStorage, repository);
    // Real source manager
    const sourceManager = new SourceManager(ingestPipeline, repository, vectorStorage);
    return {
        db,
        repository,
        parserRegistry,
 ...
 */
export function generateLongText(targetChunks) {
 ...
 */
export function generateTopicText(topic, length) {
    const text = [];
    while (text.join(' ').length < length) {
        text.push(`${topic} involves various design patterns and best practices. ` +
