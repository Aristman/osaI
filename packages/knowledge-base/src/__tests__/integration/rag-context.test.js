...
describe('TC-007-3: RAG context format', () => {
    ...
    it('should format search results as valid markdown for RAG injection', async () => {
        ...
        await env.ingestPipeline.ingestDocument('/docs/k8s.txt', Buffer.from(content1, 'utf-8'), 'txt', ['devops', 'kubernetes']);
        await env.ingestPipeline.ingestDocument('/docs/docker.md', Buffer.from(content2, 'utf-8'), 'md', ['devops', 'docker']);
        await env.ingestPipeline.ingestDocument('/docs/microservices.txt', Buffer.from(content3, 'utf-8'), 'txt', ['architecture', 'microservices']);
        // Perform search
    ...
    it('should include complete source attribution in RAG context', async () => {
        ...
        await env.ingestPipeline.ingestDocument('/docs/api-design-guide.md', Buffer.from(mdContent, 'utf-8'), 'md', ['api', 'guide', 'rest']);
    ...
    it('should produce RAG context suitable for system prompt injection', async () => {
        ...
        await env.ingestPipeline.ingestDocument('/docs/observability.txt', Buffer.from(content, 'utf-8'), 'txt', ['observability']);
        // Search
...
//# sourceMappingURL=rag-context.test.js.map