...
describe('TC-007-4: Large document (10+ chunks)', () => {
  ...
  it('should ingest document producing more than 10 chunks and verify all stored', async () => {
    ...
    const result = await env.ingestPipeline.ingestDocument(
      '/docs/large-document.txt',
      buffer,
      'txt',
      ['large', 'test'],
    );
    ...
    // -----------------------------------------------------------------------
    expect((env.vectorStorage as any).size()).toBeGreaterThanOrEqual(result.chunksStored);
  });
  ...
  it('should return search results from correct chunks of a large document', async () => {
    ...
    const hasIntroContent = introResults.some(r =>
      r.chunk.content.toLowerCase().includes('pipeline') ||
    ...
    const hasErrorContent = errorResults.some(r =>
      r.chunk.content.toLowerCase().includes('error') ||
    ...
    const hasPerfContent = perfResults.some(r =>
      r.chunk.content.toLowerCase().includes('performance') ||
