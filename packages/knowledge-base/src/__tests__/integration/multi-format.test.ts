...
describe('TC-007-2: Cross-format search', () => {
  ...
  it('should ingest txt + md documents and search across all formats', async () => {
    ...
    const txtResult = await env.ingestPipeline.ingestDocument(
      '/docs/ml-algorithms.txt',
      txtBuffer,
      'txt',
      ['ml', 'algorithms'],
    );
    ...
    const mdResult = await env.ingestPipeline.ingestDocument(
      '/docs/ml-overview.md',
      mdBuffer,
      'md',
      ['ml', 'reference'],
