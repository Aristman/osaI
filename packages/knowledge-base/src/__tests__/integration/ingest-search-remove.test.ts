...
describe('TC-007-1: Full cycle ingest -> search -> remove', () => {
  ...
  it('should complete full lifecycle: ingest -> search with attribution -> remove -> verify deleted', async () => {
    ...
    const ingestResult = await env.ingestPipeline.ingestDocument(
      '/docs/rest-api-guide.txt',
      buffer,
      'txt',
      ['api', 'guide'],
    );
    ...
    const allResults = env.vectorStorage.search(
  ...
  it('should support ingesting multiple documents and searching across them', async () => {
    ...
    const result1 = await env.ingestPipeline.ingestDocument(
      '/docs/database.txt',
    ...
    const result2 = await env.ingestPipeline.ingestDocument(
      '/docs/auth.txt',
  ...
  it('should throw SourceNotFoundError when removing non-existent document', async () => {
    await expect(
      env.sourceManager.removeSource('non-existent-id'),
