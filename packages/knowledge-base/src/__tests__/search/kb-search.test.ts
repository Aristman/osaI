...
import type { EmbeddingProvider, VectorStorage, EmbeddingResult } from '@osai/memory';
...
function createMockEmbeddingProvider(): EmbeddingProvider {
  const embedFn = vi.fn(async (text: string | string[]): Promise<EmbeddingResult | EmbeddingResult[]> => {
    // Return a deterministic vector based on text length for reproducibility
    const input = Array.isArray(text) ? text : [text];
    const results: EmbeddingResult[] = input.map(t => ({
...
function createMockVectorStorage(
  ...
  return {
    ...
    upsert: vi.fn(),
    delete: vi.fn(),
    search: vi.fn((_queryVector: number[], _topK: number, _minSimilarity: number) => results),
  };
}
...
function insertTestDocument(
  ...
  repository.insertDocument({
    ...
    title: doc.title,
    path: doc.path,
    format: doc.format as 'txt' | 'md' | 'pdf',
    status: 'completed',
...
describe('KBSearch', () => {
  ...
  describe('TC-005-2: Search filters by min_similarity', () => {
    it('should only return results with similarity >= minSimilarity', async () => {
      ...
      expect(vectorStorage.search).toHaveBeenCalledWith(
        expect.any(Array),
        expect.any(Number),
        0.7,
      );
  ...
  describe('TC-005-6: RAG formatting produces valid markdown', () => {
    it('should format results as markdown with sources and content', () => {
      const results: import('../../types/search.js').KBSearchResult[] = [
        {
          chunk: {
            id: 'chunk-1-0',
