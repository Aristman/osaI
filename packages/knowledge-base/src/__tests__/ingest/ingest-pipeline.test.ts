...
import type { EmbeddingProvider } from '@osai/memory';
import type { VectorStorage } from '@osai/memory';
import type { EmbeddingResult } from '@osai/memory';

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function createMockEmbeddingProvider(failOnIndex?: number): EmbeddingProvider {
  const embedFn = vi.fn(async (texts: string | string[]): Promise<EmbeddingResult | EmbeddingResult[]> => {
...
function createMockVectorStorage(): VectorStorage & { _entries: Map<string, { vector: number[]; metadata: Record<string, string | number | boolean> }> } {
  const entries = new Map<string, { vector: number[]; metadata: Record<string, string | number | boolean> }>();

  return {
    init: vi.fn(),
    upsert: vi.fn((id: string, vector: number[], metadata: Record<string, string | number | boolean>) => {
      entries.set(id, { vector, metadata });
    }),
    delete: vi.fn((id: string) => {
      entries.delete(id);
    }),
    search: vi.fn((_queryVector: number[], _topK: number, _minSimilarity: number) => []),
    _entries: entries,
  };
}
...
describe('IngestPipeline', () => {
  ...
  let vectorStorage: ReturnType<typeof createMockVectorStorage>;
  ...
  describe('TC-004-6: Pipeline rolls back on embed failure', () => {
    it('should roll back all data when embedding fails', async () => {
      ...
      const failingPipeline = new IngestPipeline(
        repository,
        parserRegistry,
        failingProvider,
        failingStorage,
      ...
      await expect(
        failingPipeline.ingestDocument('/test/fail.txt', buffer, 'txt', ['test']),
  ...
  describe('TC-004-8: Pipeline handles duplicate document', () => {
    it('should throw DocumentAlreadyExistsError for duplicate path', async () => {
      ...
      await expect(
        pipeline.ingestDocument('/test/dup.txt', buffer, 'txt'),
