...
import { NotificationService, type NotifierAdapter, type NotifierAdapterOptions } from "../../notifications/notification-service.js";

function getCallOpts(adapter: NotifierAdapter): NotifierAdapterOptions {
  return vi.mocked(adapter.notify).mock.calls[0]![0];
}

describe("NotificationService", () => {
  let adapter: NotifierAdapter;
  ...
  describe("notify", () => {
    ...
    it("should return ok: false on adapter error (graceful degradation)", async () => {
      vi.mocked(adapter.notify).mockImplementation(
        (_opts: unknown, callback: (err: Error | null, response: unknown) => void) => {
