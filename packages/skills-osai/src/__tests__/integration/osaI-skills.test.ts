...
import type { KnowledgeBaseService } from '../../skills/knowledge-base/KnowledgeBaseSkill.js';
import { ChatManagementSkill } from '../../skills/chat-management/ChatManagementSkill.js';
import type { ChatGatewayService } from '../../skills/chat-management/ChatManagementSkill.js';
import { OsIntegrationSkill } from '../../skills/os-integration/OsIntegrationSkill.js';
import type { OsIntegrationService } from '../../skills/os-integration/OsIntegrationSkill.js';
...
function createMockKBService(): KnowledgeBaseService {
  return {
...
function createMockChatService(): ChatGatewayService {
  return {
...
function createMockOsService(): OsIntegrationService {
  return {
...
describe('Integration: osaI Skills (T-009)', () => {
  ...
  describe('All osaI skills register in registry', () => {
    it('registers Memory skill', () => {
      const skill = new MemorySkill({ memoryService: new MemoryService() as any });
  ...
  describe('Tool definitions', () => {
    let allTools: ReturnType<SkillRegistry['getTools']>;

    beforeEach(() => {
      registry.register(new MemorySkill({ memoryService: new MemoryService() as any }).getDefinition());
  ...
  describe('Permission flow for all osaI tools', () => {
    it('Memory tools all have confirm permission', () => {
      const memorySkill = new MemorySkill({ memoryService: new MemoryService() as any });
  ...
  describe('Execute via registry', () => {
    it('Memory: recall returns empty results through registry', async () => {
      ...
      const inner = result.data as { success: boolean; data?: unknown };
    ...
    it('OS: get_system_info returns data through registry', async () => {
      ...
      expect(result.success).toBe(true);
      const inner = result.data as { success: boolean; data: { cpu: { model: string } } };
      expect(inner.success).toBe(true);
