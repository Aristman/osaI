...
import type { OsIntegrationService } from '../OsIntegrationSkill.js';

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

function createMockOsIntegrationService(): OsIntegrationService {
  return {
    notify: vi.fn(),
    listProcesses: vi.fn(),
    getSystemInfo: vi.fn(),
...
describe('OsIntegrationSkill', () => {
  let skill: OsIntegrationSkill;
  let mockService: OsIntegrationService;
  ...
  describe('list_processes (TC-008-7)', () => {
    it('delegates to OsIntegrationService.listProcesses()', async () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        const data = result.data as Array<{ pid: number; name: string }>;
  ...
  describe('get_system_info (TC-008-8)', () => {
    it('delegates to OsIntegrationService.getSystemInfo()', async () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        const data = result.data as typeof systemInfo;
