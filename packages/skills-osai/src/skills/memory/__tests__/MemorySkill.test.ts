...
function createMockStoredEntry(overrides?: Partial<MemoryEntry>): MemoryEntry {
...
describe('MemorySkill', () => {
  ...
  describe('remember (TC-007-1)', () => {
    ...
    it('stores with long-term tier', async () => {
      ...
      const callArgs = mockStore.mock.calls[0] as [MemoryEntry, string];
  ...
  describe('recall (TC-007-2)', () => {
    it('delegates to MemoryService.query() with correct parameters', async () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        const data = result.data as Array<{ id: string; content: string; similarity: number }>;
  ...
  describe('summarize_session (TC-007-4)', () => {
    it('delegates to MemoryService.query() with session context (placeholder)', async () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        const data = result.data as { sessionId: string; summary: string; facts: unknown[] };
