...
describe('HookRegistry', () => {
    ...
    describe('execute', () => {
        ...
        it('should pass modified context from one handler to the next', async () => {
            ...
            const context = createBaseContext();
            const result = await registry.execute(HookPoint.BEFORE_CONTEXT_ASSEMBLY, context);
    ...
    describe('unregister', () => {
        ...
        it('should return false when trying to unregister non-existent handler', () => {
            const removed = registry.unregister(HookPoint.BEFORE_INTAKE, 'non-existent-id');
            expect(removed).toBe(false);
        });
        it('should return false when unregistering from hook point with no handlers', () => {
            const removed = registry.unregister(HookPoint.AFTER_INTAKE, 'some-id');
            expect(removed).toBe(false);
        });
        it('should only remove the specified handler, not others', async () => {
            const handler1 = vi.fn((ctx) => ctx);
            const handler2 = vi.fn((ctx) => ctx);
    ...
    describe('empty state', () => {
        ...
        it('should return the same context for a hook point that was never used', async () => {
            const context = createBaseContext();
            const result = await registry.execute(HookPoint.ON_CHAT_SWITCH, context);
            expect(result).toBe(context);
        });
    });
    // -----------------------------------------------------------------------
    ...
    describe('HookContext', () => {
        ...
        it('should allow handler to extend data', async () => {
            ...
            const result = await registry.execute(HookPoint.BEFORE_FACT_EXTRACTION, context);
    ...
    describe('HookPoint enum', () => {
        ...
        it('should contain all hook points covering the full pipeline', () => {
            ...
            const actualValues = Object.values(HookPoint);
    ...
    describe('error handling (graceful degradation)', () => {
        it('should continue execution when a handler throws a sync error', async () => {
            ...
            const succeedingHandler = vi.fn((ctx) => ({
                ...ctx,
            ...
            expect(consoleSpy.mock.calls[0][0]).toContain('[HookRegistry] Handler');
        ...
        it('should continue execution when a handler throws an async error', async () => {
            const consoleSpy = vi
                .spyOn(console, 'error')
                .mockImplementation(() => { });
            const asyncThrowingHandler = async () => {
            ...
            registry.register(HookPoint.AFTER_RESPONSE_STREAMING, asyncThrowingHandler);
            registry.register(HookPoint.AFTER_RESPONSE_STREAMING, succeedingHandler);
            const context = createBaseContext();
            const result = await registry.execute(HookPoint.AFTER_RESPONSE_STREAMING, context);
            expect(succeedingHandler).toHaveBeenCalledTimes(1);
        ...
        it('should handle non-Error thrown values gracefully', async () => {
            ...
            const succeedingHandler = vi.fn((ctx) => ctx);
            registry.register(HookPoint.ON_DESKTOP_NOTIFICATION, throwingString);
            registry.register(HookPoint.ON_DESKTOP_NOTIFICATION, succeedingHandler);
            const context = createBaseContext();
        ...
        it('should continue when ALL handlers throw', async () => {
            ...
            const context = createBaseContext();
            const result = await registry.execute(HookPoint.ON_MIRROR_MESSAGE, context);
    ...
    describe('clear', () => {
        it('should remove all handlers from all hook points', async () => {
            ...
            expect(registry.getHandlerCount(HookPoint.BEFORE_CONTEXT_ASSEMBLY)).toBe(0);
    ...
    describe('getHandlerCount', () => {
        ...
        it('should return correct count after register and unregister', () => {
            const id = registry.register(HookPoint.BEFORE_INTAKE, () => undefined);
            expect(registry.getHandlerCount(HookPoint.BEFORE_INTAKE)).toBe(1);
