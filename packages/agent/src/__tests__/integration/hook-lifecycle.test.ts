...
import type { LLMProvider, ChatMessage, ToolCall } from '@osai/providers';
import type { RAGQueryFn, RAGResult } from '../../context/types.js';
...
describe('Integration: Hook Lifecycle (TC-008-3, TC-008-4)', () => {
  ...
  it('TC-008-3: hooks called in correct order for tool execution loop', async () => {
    ...
    const registry = {
      execute: vi.fn<() => Promise<ToolResult>>().mockResolvedValue({
        success: true,
        data: '12:00',
      }),
    };
  ...
  it('TC-008-4: BEFORE_FACT_EXTRACTION and AFTER_MEMORY_QUERY hooks called in fact extraction', async () => {
    ...
    await extractor.extract(
      'My name is Alice. The deadline is 2026-06-01.',
      'chat-hook-003',
      'session-hook-003',
    );
  ...
  it('should pass correct correlation IDs through all hooks', async () => {
    ...
    const ragQuery: RAGQueryFn = vi.fn().mockResolvedValue([]);
