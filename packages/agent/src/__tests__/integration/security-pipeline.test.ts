...
function createContext(data: Record<string, unknown>, contextOverrides?: Partial<Pick<HookContext, 'traceId' | 'sessionId' | 'chatId'>>): HookContext {
  return {
    hookPoint: HookPoint.BEFORE_TOOL_EXECUTION,
    sessionId: contextOverrides?.sessionId ?? 'integration-session',
    chatId: contextOverrides?.chatId ?? 'integration-chat',
    traceId: contextOverrides?.traceId ?? 'integration-trace',
...
describe('Security Pipeline Integration', () => {
  let audit: ReturnType<typeof createInMemoryAuditService>;
  let fileSandbox: FileSandbox;
  ...
  it('TC-008-6: full pipeline -- file op allowed -> sandbox check -> audit', async () => {
    ...
    registry.register(
      ...
      createOnFileAccessAudit({ auditService: audit.auditService }),
      10,
    );
    ...
    const toolCallRecord = audit.records.find(
      (r) => r.action === AuditEventType.TOOL_CALL,
    );
    ...
    const fileAccessRecord = audit.records.find(
      (r) => r.action === AuditEventType.FILE_ACCESS,
    );
  ...
  it('full pipeline -- shell exec allowed -> command validated -> audit', async () => {
    ...
    registry.register(
      HookPoint.AFTER_TOOL_EXECUTION,
      createAfterToolCallAudit({ auditService: audit.auditService }),
      10,
    );
    ...
    const shellExecRecord = audit.records.find(
      (r) => r.action === AuditEventType.SHELL_EXEC,
    );
  ...
  it('full pipeline -- sandbox violation -> blocked, no after hook audit', async () => {
    ...
    // Try to read a file outside sandbox (not blocked by pattern, but by allowed_dirs)
    let ctx = createContext({
      tool_name: 'read_file',
      tool_params: { path: '/var/log/some.log' },
    });
    ...
    const violationRecord = audit.records.find(
      (r) => r.action === AuditEventType.SANDBOX_VIOLATION,
    );
  ...
  it('full pipeline -- blocked command -> denied', async () => {
    ...
    const shellRecord = audit.records.find(
      (r) => r.action === AuditEventType.SHELL_EXEC,
    );
    expect(shellRecord).toBeDefined();
    expect(shellRecord!.result).toEqual(
