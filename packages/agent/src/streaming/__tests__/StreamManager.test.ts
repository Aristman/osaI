...
function createChunk(overrides?: Partial<InferenceChunk>): InferenceChunk {
  return {
    content: 'Hello',
    model: 'glm-5',
    provider: 'z-ai',
 ...
 */
async function* streamFromChunks(chunks: InferenceChunk[]): AsyncGenerator<InferenceChunk> {
...
describe('StreamManager', () => {
  ...
  describe('processStream', () => {
    it('TC-006-1: should call callback for each chunk', async () => {
      ...
      // First call
      const call0 = callback.mock.calls[0]![0] as StreamChunk;
      ...
      // Second call
      const call1 = callback.mock.calls[1]![0] as StreamChunk;
      expect(call1.content).toBe(' world');

      // Third call
      const call2 = callback.mock.calls[2]![0] as StreamChunk;
    ...
    it('TC-006-2: should emit complete event after stream ends', async () => {
      ...
      const event: StreamEvent = listener.mock.calls[0]![0];
    ...
    it('should reset accumulated text on new processStream call', async () => {
      const chunks1: InferenceChunk[] = [createChunk({ content: 'First' })];
      const chunks2: InferenceChunk[] = [createChunk({ content: 'Second' })];
    ...
    it('should pass chunks with tool calls to callback', async () => {
      ...
      const streamChunk = callback.mock.calls[0]![0] as StreamChunk;
  ...
  describe('event listeners', () => {
    it('should support multiple listeners for complete event', async () => {
      ...
      const chunks: InferenceChunk[] = [createChunk({ content: 'Test' })];
    ...
    it('should support chunk event', async () => {
      ...
      const event0 = chunkListener.mock.calls[0]![0] as StreamChunk;
      expect(event0.content).toBe('Hi');
      const event1 = chunkListener.mock.calls[1]![0] as StreamChunk;
