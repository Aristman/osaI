...
function createTestInput(overrides?: Partial<InferenceInput>): InferenceInput {
  return {
    messages: [
...
function createMockLLMChunk(overrides?: Partial<LLMChunk>): LLMChunk {
  return {
...
describe('InferenceService', () => {
  ...
  describe('TC-003-1: Non-streaming inference', () => {
    it('should return InferenceResult with content, usage, model, provider', async () => {
      ...
      const result: InferenceResult = await service.infer(input);
  ...
  describe('TC-003-2: Streaming inference', () => {
    it('should yield InferenceChunks from the provider stream', async () => {
      ...
      vi.mocked(mockProvider.stream).mockReturnValue(
        (async function* () {
      ...
      const input = createTestInput();
      const collectedChunks: InferenceChunk[] = [];
  ...
  describe('TC-003-3: BEFORE_MODEL_INFERENCE hook', () => {
    it('should call BEFORE_MODEL_INFERENCE hook before provider.complete', async () => {
      const handler = vi.fn<import('../../hooks/types.js').HookHandler>(
        (ctx) => ctx,
      );
    ...
    it('should allow hook to modify model in request', async () => {
      const modifierHandler: import('../../hooks/types.js').HookHandler = (
        ctx,
      ) => ({
        ...ctx,
    ...
    it('should call AFTER_MODEL_INFERENCE hook after provider.complete', async () => {
      const afterHandler = vi.fn<import('../../hooks/types.js').HookHandler>(
        (ctx) => ctx,
      );
    ...
    it('should call hooks in correct order: before then after', async () => {
      ...
      const beforeHandler: import('../../hooks/types.js').HookHandler = (
        ctx,
      ) => {
        callOrder.push('before');
        return ctx;
      };
      const afterHandler: import('../../hooks/types.js').HookHandler = (ctx) => {
  ...
  describe('TC-003-5: Error handling', () => {
    ...
    it('should still call BEFORE_MODEL_INFERENCE hook even if provider throws', async () => {
      ...
      vi.mocked(mockProvider.complete).mockRejectedValue(
    ...
    it('should NOT call AFTER_MODEL_INFERENCE hook if provider throws', async () => {
      const beforeHandler = vi.fn<import('../../hooks/types.js').HookHandler>(
        (ctx) => ctx,
      );
      const afterHandler = vi.fn<import('../../hooks/types.js').HookHandler>(
        (ctx) => ctx,
  ...
  describe('Edge cases', () => {
    ...
    it('should preserve traceId, chatId, sessionId in hook contexts', async () => {
      ...
      const captureHandler: import('../../hooks/types.js').HookHandler = (
        ctx,
      ) => {
