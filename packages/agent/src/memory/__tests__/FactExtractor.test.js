...
describe('FactExtractor', () => {
    ...
    describe('TC-007-1: Extracts facts from assistant response', () => {
        it('should extract dates in ISO format', async () => {
            const facts = await extractor.extract('The meeting is scheduled for 2026-03-30.', 'chat-001', 'session-001');
        ...
        it('should extract dates in long format', async () => {
            const facts = await extractor.extract('The deadline is March 15, 2026.', 'chat-001', 'session-001');
            const dateFacts = facts.filter((f) => f.category === 'date');
            expect(dateFacts.length).toBeGreaterThanOrEqual(1);
        });
        it('should extract names', async () => {
            const facts = await extractor.extract('My name is Alex and I work as a developer.', 'chat-001', 'session-001');
        ...
        it('should extract numbers with units', async () => {
            const facts = await extractor.extract('The project budget is 5000 USD and the deadline is in 30 days.', 'chat-001', 'session-001');
        ...
        it('should extract preferences', async () => {
            const facts = await extractor.extract('I prefer dark mode for my IDE. I always use Vim for editing.', 'chat-001', 'session-001');
        ...
        it('should extract explicit facts', async () => {
            const facts = await extractor.extract('Remember that the API key expires on June 1st.', 'chat-001', 'session-001');
        ...
        it('should assign correct metadata to extracted facts', async () => {
            const facts = await extractor.extract('My name is Alice.', 'chat-123', 'session-456');
        ...
        it('should deduplicate identical facts', async () => {
            const facts = await extractor.extract('The date is 2026-03-30. Remember: the date is 2026-03-30.', 'chat-001', 'session-001');
            const dateFacts = facts.filter((f) => f.category === 'date');
    ...
    describe('TC-007-2: AFTER_MEMORY_QUERY hook called with facts', () => {
        it('should call AFTER_MEMORY_QUERY hook after extraction', async () => {
            ...
            await extractor.extract('My name is Bob. The deadline is 2026-04-01.', 'chat-001', 'session-001');
        ...
        it('should include extracted facts count in hook data', async () => {
            ...
            await extractor.extract('My name is Charlie.', 'chat-001', 'session-001');
        ...
        it('should include fact categories in hook data', async () => {
            ...
            await extractor.extract('My name is Dave. The deadline is 2026-05-01.', 'chat-001', 'session-001');
            const ctx = hookCalls[0];
            const categories = ctx.data['factCategories'];
        ...
        it('should call BEFORE_FACT_EXTRACTION hook before extraction', async () => {
            ...
            await extractor.extract('Some response text.', 'chat-001', 'session-001');
            expect(hookCalls).toHaveLength(1);
    ...
    describe('TC-007-3: Delegates saving to Memory System', () => {
        it('should store extracted facts via storeFacts function', async () => {
            const facts = await extractor.extract('My name is Eve.', 'chat-001', 'session-001');
        ...
        it('should not call storeFacts when no facts extracted', async () => {
            const facts = await extractor.extract('Hello, how are you?', 'chat-001', 'session-001');
            await extractor.storeFacts(facts);
    ...
    describe('TC-007-4: Error handling (graceful degradation)', () => {
        it('should return empty facts and not throw when hook throws', async () => {
            ...
            const facts = await extractor.extract('My name is Frank.', 'chat-001', 'session-001');
        ...
        it('extractAndStore should handle extraction errors gracefully', async () => {
            ...
            const result = await extractor.extractAndStore('Normal text without facts.', 'chat-001', 'session-001');
        ...
        it('extractAndStore should not report errors when storeFacts fails (graceful degradation in storeFacts)', async () => {
            ...
            const result = await extractor.extractAndStore('My name is Grace.', 'chat-001', 'session-001');
    ...
    describe('TC-007-5: Empty response', () => {
        it('should return empty array for empty response', async () => {
            const facts = await extractor.extract('', 'chat-001', 'session-001');
            expect(facts).toHaveLength(0);
        });
        it('should return empty array for response with no patterns', async () => {
            const facts = await extractor.extract('Hello, how can I help you today?', 'chat-001', 'session-001');
            expect(facts).toHaveLength(0);
        });
        it('should return empty array for whitespace-only response', async () => {
            const facts = await extractor.extract('   \n\t  ', 'chat-001', 'session-001');
            expect(facts).toHaveLength(0);
        });
        it('extractAndStore should return count=0 for empty response', async () => {
            const result = await extractor.extractAndStore('No facts here.', 'chat-001', 'session-001');
    ...
    describe('extractAndStore', () => {
        it('should extract and store in a single call', async () => {
            const result = await extractor.extractAndStore('My name is Heidi. The deadline is 2026-06-15.', 'chat-001', 'session-001');
            expect(result.count).toBeGreaterThanOrEqual(1);
            expect(result.hasErrors).toBe(false);
            expect(mockStoreFacts).toHaveBeenCalledTimes(1);
        });
        it('should return structured ExtractionResult', async () => {
            const result = await extractor.extractAndStore('I prefer coffee over tea.', 'chat-001', 'session-001');
...
//# sourceMappingURL=FactExtractor.test.js.map