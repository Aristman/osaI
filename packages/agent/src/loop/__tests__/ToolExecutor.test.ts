...
import type { ToolExecutorConfig, ToolExecutorInput, ToolExecutorOutput } from '../ToolExecutor.js';
import type { InferenceResult, InferenceService } from '../../inference/index.js';
...
function createMockInferenceResult(
...
function createMockInferenceService(
  results: InferenceResult[],
): InferenceService {
  let callIndex = 0;

  return {
    infer: vi.fn<InferenceService['infer']>().mockImplementation(async () => {
...
function createSuccessfulToolResult(data: unknown): ToolResult {
  return { success: true, data };
}

function createFailedToolResult(error: string): ToolResult {
...
describe('ToolExecutor', () => {
  ...
  let mockInferenceService: InferenceService;
  let mockSkillRegistry: MockSkillRegistry;
  ...
  describe('TC-005-7: No tool_use -> single inference pass', () => {
    it('should return immediately when inference has no tool calls', async () => {
      ...
      const executor = new ToolExecutor(
        ...
        mockSkillRegistry as unknown as { execute: (name: string, params: Record<string, unknown>) => Promise<ToolResult> },
        hooks,
      );
      ...
      const output: ToolExecutorOutput = await executor.executeToolLoop(
        input.messages,
        input.tools,
        config,
      );
  ...
  describe('TC-005-1: tool_use -> execute -> result -> re-inference', () => {
    it('should execute tool and re-call inference when tool_use response', async () => {
      ...
      mockSkillRegistry.execute.mockResolvedValue(
        createSuccessfulToolResult({ content: 'hello world' }),
      );
      ...
      expect((mockInferenceService.infer as ReturnType<typeof vi.fn>).mock.calls.length).toBe(2);
    ...
    it('should pass tool results back to inference in subsequent calls', async () => {
      ...
      mockInferenceService = createMockInferenceService([toolCallResponse, finalResponse]);
      mockSkillRegistry.execute.mockResolvedValue(
        createSuccessfulToolResult({ temp: 20, condition: 'sunny' }),
      );
      ...
      const secondCall = (mockInferenceService.infer as ReturnType<typeof vi.fn>).mock.calls[1]!;
      const secondMessages = secondCall[0].messages as ChatMessage[];
      ...
      expect(toolMessage.content).toBe(
        JSON.stringify({ success: true, data: { temp: 20, condition: 'sunny' } }),
      );
    });
  });
  ...
  describe('TC-005-3: before_tool_execution hook', () => {
    it('should call BEFORE_TOOL_EXECUTION hook before each tool call', async () => {
      ...
      mockInferenceService = createMockInferenceService([toolCallResponse, finalResponse]);
      mockSkillRegistry.execute.mockResolvedValue(
        createSuccessfulToolResult(null),
      );
  ...
  describe('TC-005-4: after_tool_execution hook', () => {
    it('should call AFTER_TOOL_EXECUTION hook after each tool call', async () => {
      ...
      mockInferenceService = createMockInferenceService([toolCallResponse, finalResponse]);
      mockSkillRegistry.execute.mockResolvedValue(
        createSuccessfulToolResult({ items: ['a', 'b'] }),
      );
      ...
      expect(hookCalls[0]!.data['toolName']).toBe('search');
      expect(hookCalls[0]!.data['toolCallId']).toBe('call-001');
      expect(hookCalls[0]!.data['toolResult']).toEqual(
        { success: true, data: { items: ['a', 'b'] } },
      );
    });
    ...
    it('should call AFTER_TOOL_EXECUTION with error info when tool fails', async () => {
      ...
      mockInferenceService = createMockInferenceService([toolCallResponse, finalResponse]);
      mockSkillRegistry.execute.mockResolvedValue(
        createFailedToolResult('Permission denied'),
      );
  ...
  describe('TC-005-5: Tool result appended to messages', () => {
    it('should append tool result message with role=tool and tool_call_id', async () => {
      ...
      mockInferenceService = createMockInferenceService([toolCallResponse, finalResponse]);
      mockSkillRegistry.execute.mockResolvedValue(
        createSuccessfulToolResult('2026-03-30T12:00:00Z'),
      );
      ...
      expect(toolMessages[0]!.content).toBe(
        JSON.stringify({ success: true, data: '2026-03-30T12:00:00Z' }),
      );

      // Also check that assistant message with tool_calls is present
      const assistantWithTools = output.messages.find(
  ...
  describe('TC-005-6: Max iterations guard', () => {
    it('should stop after max iterations (default 10)', async () => {
      ...
      expect((mockInferenceService.infer as ReturnType<typeof vi.fn>).mock.calls.length).toBe(10);
    });
    ...
    it('should respect custom maxIterations value', async () => {
      ...
      expect(output.maxIterationsReached).toBe(true);
      expect((mockInferenceService.infer as ReturnType<typeof vi.fn>).mock.calls.length).toBe(3);
    });
  ...
  describe('TC-005-8: Tool execution error handling', () => {
    it('should continue loop when SkillRegistry.execute throws', async () => {
      ...
      mockInferenceService = createMockInferenceService([toolCallResponse, finalResponse]);
      mockSkillRegistry.execute.mockRejectedValue(
        new Error('Tool handler crashed'),
      );
    ...
    it('should still call after_tool_execution hook when tool fails', async () => {
      ...
      const afterHookCalls: HookContext[] = [];
      ...
      expect(afterHookCalls[0]!.data['toolResult']).toEqual(
        { success: false, error: 'Crash' },
      );
  ...
  describe('Multi-iteration tool loop', () => {
    it('should handle 3 iterations of tool calls', async () => {
      const responses: InferenceResult[] = [
