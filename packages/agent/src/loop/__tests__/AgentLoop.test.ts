...
function createTestInput(overrides?: Partial<AgentLoopInput>): AgentLoopInput {
  return {
...
function createMockAssemblyResult(
...
function createMockContextAssembler(
  ...
  return {
    assemble: vi.fn<ContextAssembler['assemble']>().mockResolvedValue(
      assemblyResult,
    ),
...
function createMockInferenceService(
  ...
  return {
    infer: vi.fn<InferenceService['infer']>().mockResolvedValue(
      inferenceResult,
    ),
    inferStream: vi.fn(),
...
describe('AgentLoop', () => {
  ...
  describe('TC-004-1: run() executes full pipeline', () => {
    it('should return AgentLoopOutput with content, usage, model, provider', async () => {
      ...
      const result: AgentLoopOutput = await loop.run(input);
    ...
    it('should call context.assemble() and inference.infer() in order', async () => {
      ...
      const assemblerSpy = vi.spyOn(assembler, 'assemble').mockImplementation(
        async () => {
      ...
      const inferencerSpy = vi.spyOn(inferencer, 'infer').mockImplementation(
        async () => {
    ...
    it('should pass correct parameters to context.assemble()', async () => {
      ...
      expect(assemblerSpy).toHaveBeenCalledWith(
    ...
    it('should pass assembled messages to inference.infer()', async () => {
      ...
      expect(inferencerSpy).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('should pass config defaults to inference.infer()', async () => {
      ...
      const configWithDefaults: AgentLoopConfig = {
        systemPrompt: 'Test',
      ...
      const loop = new LoopWithAssembler(
        configWithDefaults,
        mockContextAssembler,
        inferencer,
        hooks,
    ...
    it('should return ragResultCount from context assembly', async () => {
      const assembler = createMockContextAssembler(
        createMockAssemblyResult({ ragResultCount: 3 }),
      );
    ...
    it('should generate traceId when not provided in input', async () => {
      ...
      const assemblerSpy = vi.spyOn(
        (loop as unknown as { _contextAssembler: { assemble: unknown } })._contextAssembler as { assemble: (...args: unknown[]) => unknown },
        'assemble',
      );
    });
  });
  ...
  describe('TC-004-4: Error handling', () => {
    it('should return error response when inference throws', async () => {
      ...
      const inferencer = createMockInferenceService();
      vi.spyOn(inferencer, 'infer').mockRejectedValue(
    ...
    it('should return error response when context.assemble() throws', async () => {
      ...
      const assembler = createMockContextAssembler();
      vi.spyOn(assembler, 'assemble').mockRejectedValue(
      ...
      const inferencer = mockInferenceService as {
  ...
  describe('TC-004-7: trace_id preserved through pipeline', () => {
    ...
    it('should pass correlation IDs (sessionId, chatId) to assemble and infer', async () => {
      let assembleInput: { sessionId: string; chatId: string } | undefined;
      let inferInput: { sessionId: string; chatId: string } | undefined;
  ...
  describe('Edge cases', () => {
    ...
    it('should return finishReason from inference', async () => {
      const inferencer = createMockInferenceService(
        createMockInferenceResult({ finishReason: 'tool_calls' }),
      );
