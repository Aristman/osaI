 ...
 */
export function startMockLlmServer(config: MockLlmServerConfig = {}): Promise<MockLlmServer> {
  const {
    responseContent: initialContent = "Mock LLM response",
    delayMs = 0,
    shouldFail: initialFail = false,
    failStatusCode = 500,
    model = "mock-llm",
  } = config;
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      resolve({
        ...
        baseUrl,
        get requests() {
          return recordedRequests as readonly RecordedRequest[];
        },
 ...
 */
function handleStreaming(res: ServerResponse, content: string, model: string): void {
  res.writeHead(200, {
...
function findLastUserContent(messages: Array<{ role?: string; content?: string }>): string {
