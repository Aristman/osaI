...
- **Z.ai -- верифицирован** -- Z.ai подтверждён как реальный LLM-провайдер с OpenAI-совместимым API. Endpoint: `https://api.z.ai/api/paas/v4`, модель: `glm-5`, streaming, Bearer auth. Работает через OpenAI Node.js SDK с кастомным `baseURL`. Документация: https://docs.z.ai/api-reference/introduction
...
|---|---|---|
| **Z.ai** | ВЕРИФИЦИРОВАН -- реальный провайдер | OpenAI-совместимый API. Endpoint: `https://api.z.ai/api/paas/v4`. Модель: `glm-5`. Поддержка streaming, Bearer auth. Работает через OpenAI Node.js SDK с кастомным `baseURL` |
...
#### R1: Z.ai -- верифицирован, но требует тестирования стабильности

- **Описание:** Z.ai подтверждён как реальный провайдер (верифицировано 2026-03-30). Endpoint: `https://api.z.ai/api/paas/v4`, модель: `glm-5`, OpenAI-совместимый API. Работает через OpenAI Node.js SDK. Остаточный риск: стабильность API, uptime, поддержка tool calling, rate limits
- **Вероятность:** Низкая (15% -- основная неопределённость снята)
- **Влияние:** Среднее -- при нестабильности Z.ai failover chain автоматически переключится на Yandex/Anthropic/OpenAI/Ollama
- **Митигация:** 5-уровневая failover chain, circuit breaker, auth profile rotation. Z.ai как primary, но не единственный провайдер
...
**Q1: Z.ai -- верифицирован (RESOLVED)**
- Z.ai подтверждён как реальный LLM-провайдер (верифицировано 2026-03-30)
- Endpoint: `https://api.z.ai/api/paas/v4`
- Модель: `glm-5`
- OpenAI-совместимый API: работает через OpenAI Node.js SDK с кастомным `baseURL`
- Поддержка: streaming, Bearer auth, tool calling
- Документация: https://docs.z.ai/api-reference/introduction
- **Статус:** РЕШЁНО -- Z.ai существует и может использоваться как primary провайдер
...
| A2 | Z.ai существует как реальный LLM-провайдер | **90%** | ВЕРИФИЦИРОВАН 2026-03-30. Endpoint: https://api.z.ai/api/paas/v4, модель: glm-5, OpenAI-совместимый |
...
1. **Z.ai верифицирован** -- Z.ai подтверждён как реальный провайдер (endpoint: `https://api.z.ai/api/paas/v4`, модель: `glm-5`, OpenAI-совместимый). Риск снят. Остаточная задача: тестирование стабильности в продакшн-нагрузке
...
| "Z.ai primary LLM" | Z.ai ВЕРИФИЦИРОВАН (2026-03-30). Endpoint: `https://api.z.ai/api/paas/v4`, модель: `glm-5`, OpenAI-совместимый |
...
1. **Z.ai верифицирован** -- больше не является блокером. Endpoint: `https://api.z.ai/api/paas/v4`, модель: `glm-5`
